...
import {
  IUser,
  IUserAuthorizationData,
  IUserAuthorizationResponse,
...
export class ApiService {
  ...
  checkToken(Login: string, Token: string) {
    return this.http.post(`/api/check_token?Login=${Login}`, { Token }).pipe(
      switchMap(() => of(true)),
      catchError(() => of(false))
    );
  }
  ...
  registrationUser(
    userRegistrationData: IUserAuthorizationData
  ): Observable<IUserAuthorizationResponse | null> {
    return this.http
      .post<IAuthorizationResponse>(
        `/api/registration?Login=${userRegistrationData.login}&Password=${userRegistrationData.password}`,
        {}
      )
      .pipe(
        switchMap((response) => {
  ...
  authorizationUser(
    userAuthorizationData: IUserAuthorizationData
  ): Observable<IUserAuthorizationResponse | null> {
    return this.http
      .post<IAuthorizationResponse>(
        `/api/authorization?Login=${userAuthorizationData.login}&Password=${userAuthorizationData.password}`,
        {}
      )
      .pipe(
        switchMap((response) => {
