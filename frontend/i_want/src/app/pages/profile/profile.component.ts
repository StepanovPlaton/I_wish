...
import { ADD_IMAGE, AVATAR, CHECK, CLOSE, EDIT, EXTERNAL_LINK, LOGOUT } from 'src/app/shared/consts/images.consts';
...
export class ProfileComponent implements OnInit {
  ...
  submit() {
    if(this.authService.userAuthorizationData) {
      this.apiService.updateUserInfo(
        ...
        this.user
      ).subscribe(()=>{this.close()}, ()=>this.error = true)
    }
  }
  ...
  logout() {
    this.translateService.get(["Logout", "Cancel"]).pipe(
